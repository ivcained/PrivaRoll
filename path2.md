...
export function generateStealthPayment(employeePublicKeyHex: string) {
  ...
  return {
    stealthAddress, // HR sends USDC here via BitGo
...
async function executePayroll(employeeEnsName: string, amountUSDC: string) {
  ...
  console.log(`Sending ${amountUSDC} USDC to unlinked address: ${stealthAddress}`);
  console.log(`Ephemeral Key (to publish): ${ephemeralPublicKey}`);

  // 3. Send these details to your BitGo backend integration
  // BitGo batches the USDC transfers to 'stealthAddress'
